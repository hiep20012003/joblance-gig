...
export class GigRepository {
  ...
  search = async ({
    ...
    // 🔹 Filters
    // ======================================================
    if (categories.length) filter.push({ terms: { "categories.keyword": categories } });
    if (subCategories.length) filter.push({ terms: { "subCategories.keyword": subCategories } });
        ...
        functions: [
          {
            script_score: {
              ...
              double avgRating = ratingSum / ratingsCount;
